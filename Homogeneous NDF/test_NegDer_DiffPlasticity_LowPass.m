...
Start = now;
i = 1; % first trial
yinit = [0; Wneg*.9; 0]; % begin with a 10% perturbation
dt_store = 0.5;
[t, y]=ode23(@(t,y) odefun_NegDer_DiffPlas_LowPass(t,y,alpha,Wneg,Istim(i),TStim),0:dt_store:TAll,yinit);
nt=length(t);
Wend = zeros(n,1); % record weight balance ratio evolution with resolution of trial
Wend(1) = y(end,2)/Wneg;
ytotal = zeros(3,nt,n); % record activity time course
ytotal(:,:,1) = y';
for i = 2:n
